...
import { AllySupportRanks, Elements, MythicHeroBoostTypes, SummonerSupportRanks } from "types";
...
class MythicHeroBadge extends BaseSprite {
  ...
  render(context: Context): void {
    ...
    this.view.visible = hero.isMythicHero;
  ...
  private _getTexture(hero: State["hero"]): Texture {
    if (!hero.isMythicHero) {
      return this._defaultTexture;
    }

    const elementName = Elements[hero.element].toLowerCase();
    const mythicHeroBoostTypeName = MythicHeroBoostTypes[hero.mythicHeroBoostType].toLowerCase();

    return getTexture(`2|badge_mythic_${elementName}_${mythicHeroBoostTypeName}`);
  }
  ...
  private _getAdjustment(state: State): Adjustments | null {
    const badgeVisibilities = [
      ...
      state.blessing || state.hero.isMythicHero || state.hero.isMythicHero,
    ];
...
export default MythicHeroBadge;
